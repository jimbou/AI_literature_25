...
DBLP_PACMPL10 = "https://dblp.org/db/journals/pacmpl/pacmpl10.html"
# import requests
...
def parse_oopsla_section(issue: str) -> List[Dict[str, Any]]:
    ...
    """
    assert issue in ("OOPSLA1","POPL","PLDI","OOPSLA2")

    r = requests.get(DBLP_PACMPL10, headers={"User-Agent": UA}, timeout=60)
    r.raise_for_status()
...
def main():
    ...
    ap.add_argument("--section", choices=["popl"], default="popl")
    ...
    if args.section == "popl":
        build_json("POPL", f"{args.out_dir}/pacmpl10_popl.json", args.meta_source, args.sleep_s)
